...
- Advanced security platform modules for zero-trust session risk, tenant key metadata, security events, security posture, attack surface assets, compliance readiness, and a live Attack Lab product
...
- `GET/POST/PATCH /api/admin/attack-lab/scenarios` manages operational Attack Lab scenarios.
...
The Attack Lab is intentionally non-weaponizable. It does not run network scans, generate exploit payloads, send phishing emails, or provide step-by-step attack instructions. All runs model real-world threat sequences and operational narratives designed for readiness scoring, defensive planning, and incident response training.

Client Attack Lab features:

- Scenario catalog for credential stuffing, phishing, web app probing, and insider access readiness exercises
- Run creation through `POST /api/security-platform/client/attack-lab/runs`
...
- Visibility into client adoption of readiness exercises
