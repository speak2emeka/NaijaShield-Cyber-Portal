...
const demoTickets: Ticket[] = [
  ...
  {
    id: 'ticket-2',
    title: 'Enable phishing readiness campaign',
...
const demoSecurityEvents = [
  { id: 'sec-1', type: 'NEW_DEVICE_LOGIN', severity: 'MEDIUM', source: 'auth', message: 'New browser session observed for client administrator.', createdAt: now, clientCompany: { name: 'Lagos Fintech Group' }, metadata: { riskScore: 42, mode: 'modeled' } },
  ...
  { id: 'sec-3', type: 'ATTACK_LAB_RUN', severity: 'LOW', source: 'attack-lab', message: 'Attack Lab phishing campaign completed with readiness signal available.', createdAt: now, clientCompany: { name: 'Lagos Fintech Group' }, metadata: { mode: 'modeled' } }
];
...
const demoCsr = [
  { id: 'csr-1', clientCompanyId: 'company-1', health: 'GREEN', onboardingStage: 'LIVE', slaStatus: 'ON_TRACK', feedbackScore: 94, renewalDate: '2026-12-31T00:00:00.000Z', notes: 'Healthy account with Attack Lab adoption.', clientCompany: { id: 'company-1', name: 'Lagos Fintech Group', subscription: { plan: 'pro' }, securityPosture: { score: 84 } } },
  { id: 'csr-2', clientCompanyId: 'company-2', health: 'AMBER', onboardingStage: 'ONBOARDING', slaStatus: 'AT_RISK', feedbackScore: 78, renewalDate: '2026-09-30T00:00:00.000Z', notes: 'Needs compliance workshop.', clientCompany: { id: 'company-2', name: 'Abuja Health Network', subscription: { plan: 'enterprise' }, securityPosture: { score: 69 } } }
];
...
const demoCrmContacts: any[] = [
  { id: 'contact-1', clientCompanyId: 'company-1', name: 'Amaka Okafor', email: 'amaka@lagosfintech.test', phone: '+234 800 000 1000', title: 'Security Lead', role: 'SECURITY_LEAD', primary: true, notes: 'Primary security stakeholder', clientCompany: { id: 'company-1', name: 'Lagos Fintech Group' }, createdAt: now, updatedAt: now },
  { id: 'contact-2', clientCompanyId: 'company-2', name: 'Tunde Bello', email: 'tunde@abujahealth.test', phone: '+234 800 000 2000', title: 'IT Manager', role: 'TECHNICAL_OWNER', primary: true, notes: 'Owns implementation blockers', clientCompany: { id: 'company-2', name: 'Abuja Health Network' }, createdAt: now, updatedAt: now }
];

const demoCrmTasks: any[] = [
  { id: 'task-1', clientCompanyId: 'company-2', title: 'Schedule compliance workshop', description: 'Prepare agenda and invite compliance owner.', status: 'OPEN', priority: 'HIGH', dueDate: now, source: 'AI_ASSISTANT', clientCompany: { id: 'company-2', name: 'Abuja Health Network' }, createdAt: now, updatedAt: now },
  { id: 'task-2', clientCompanyId: 'company-1', title: 'Prepare QBR deck', description: 'Include posture trend and Attack Lab adoption.', status: 'IN_PROGRESS', priority: 'MEDIUM', dueDate: now, source: 'QBR', clientCompany: { id: 'company-1', name: 'Lagos Fintech Group' }, createdAt: now, updatedAt: now }
];

const demoCrmLifecycle: any[] = [
  { id: 'life-1', clientCompanyId: 'company-1', stage: 'QBR', summary: 'QBR preparation started', owner: 'Customer Success Manager', occurredAt: now, clientCompany: { id: 'company-1', name: 'Lagos Fintech Group' } },
  { id: 'life-2', clientCompanyId: 'company-2', stage: 'IMPLEMENTATION', summary: 'Compliance implementation workshop pending', owner: 'Technical Account Manager', occurredAt: now, clientCompany: { id: 'company-2', name: 'Abuja Health Network' } }
];

const demoRenewalOpportunities: any[] = [
  { id: 'renewal-1', clientCompanyId: 'company-1', stage: 'VALUE_REVIEW', contractValue: 4200000, probability: 82, renewalDate: '2026-12-31T00:00:00.000Z', churnRisk: 'LOW', upsellNotes: 'Consider Enterprise compliance add-on', nextStep: 'Send QBR value summary', clientCompany: { id: 'company-1', name: 'Lagos Fintech Group', customerSuccess: demoCsr[0], securityPosture: { score: 84 }, subscription: { status: 'ACTIVE' } } },
  { id: 'renewal-2', clientCompanyId: 'company-2', stage: 'DISCOVERY', contractValue: 9000000, probability: 58, renewalDate: '2026-09-30T00:00:00.000Z', churnRisk: 'MEDIUM', upsellNotes: 'Compliance package may reduce churn', nextStep: 'Run stakeholder health call', clientCompany: { id: 'company-2', name: 'Abuja Health Network', customerSuccess: demoCsr[1], securityPosture: { score: 69 }, subscription: { status: 'TRIAL' } } }
];
...
function handleDemoRequest(config: any) {
  ...
  if (method === 'get' && url === '/admin/csr/dashboard') return demoResponse(config, {
    metrics: { averageHealth: 82, openTickets: demoTickets.length, escalations: 1, slaBreaches: 1, upcomingMeetings: demoMeetings.length, renewalRisks: 1, satisfactionScore: 86 },
    clients: demoCsr.map((item, index) => ({ ...item, healthScore: index === 0 ? 88 : 71, riskLevel: index === 0 ? 'LOW' : 'MEDIUM', recommendedActions: index === 0 ? ['Maintain QBR cadence', 'Review expansion readiness'] : ['Schedule compliance workshop', 'Assign SLA owner'] })),
    recentMessages: demoMessages,
    assistant: { summary: '2 client relationships monitored across support, meetings, posture, billing, and compliance signals.', nextActions: ['Schedule Abuja compliance workshop', 'Prepare Lagos QBR agenda', 'Review SLA breach queue'], qbrPreparation: [{ client: 'Lagos Fintech Group', agenda: ['Security posture trend', 'Open operational risks', 'Compliance evidence gaps', 'Renewal plan'] }] }
  });
  if (method === 'get' && url.startsWith('/admin/csr/health')) return demoResponse(config, demoCsr.map((item, index) => ({ clientCompanyId: `company-${index + 1}`, client: item.clientCompany.name, healthScore: index === 0 ? 88 : 71, riskLevel: index === 0 ? 'LOW' : 'MEDIUM', drivers: { ticketVolume: index + 1, criticalIncidents: index, complianceGaps: index, attackSurfaceRisk: index + 1, subscriptionStatus: 'ACTIVE', postureScore: item.clientCompany.securityPosture.score }, recommendedActions: index === 0 ? ['Maintain QBR cadence'] : ['Schedule compliance workshop', 'Review SLA ownership'] })));
  if (method === 'get' && url === '/admin/csr/sla') return demoResponse(config, demoTickets.map((ticket, index) => ({ id: ticket.id, client: 'Lagos Fintech Group', title: ticket.title, priority: ticket.priority, status: ticket.status, ageHours: index ? 14 : 9, targetHours: ticket.priority === 'HIGH' ? 8 : 24, breached: index === 0 })));
  if (method === 'get' && (url === '/admin/csr/renewals' || url === '/admin/crm/renewals')) return demoResponse(config, demoRenewalOpportunities);
  if (method === 'get' && url === '/admin/csr') return demoResponse(config, demoCsr);
  if (method === 'get' && url === '/admin/crm') return demoResponse(config, demoCsr);
  if (method === 'get' && url === '/admin/crm/dashboard') return handleDemoRequest({ ...config, url: '/admin/csr/dashboard' });
  if (method === 'get' && url.startsWith('/admin/crm/health')) return handleDemoRequest({ ...config, url: '/admin/csr/health' });
  if (method === 'get' && url === '/admin/crm/sla') return handleDemoRequest({ ...config, url: '/admin/csr/sla' });
  if (method === 'post' && url === '/admin/crm') return handleDemoRequest({ ...config, url: '/admin/csr' });
  if (method === 'get' && url.startsWith('/admin/crm/contacts')) return demoResponse(config, demoCrmContacts);
  if (method === 'post' && url === '/admin/crm/contacts') { const contact = { id: crypto.randomUUID(), ...body, primary: body.primary === 'on' || body.primary === true, createdAt: now, updatedAt: now, clientCompany: { id: body.clientCompanyId, name: body.clientCompanyId === 'company-2' ? 'Abuja Health Network' : 'Lagos Fintech Group' } }; demoCrmContacts.unshift(contact); return demoResponse(config, contact, 201); }
  if (method === 'get' && url.startsWith('/admin/crm/tasks')) return demoResponse(config, demoCrmTasks);
  if (method === 'post' && url === '/admin/crm/tasks') { const task = { id: crypto.randomUUID(), status: 'OPEN', priority: 'MEDIUM', ...body, createdAt: now, updatedAt: now, clientCompany: { id: body.clientCompanyId, name: body.clientCompanyId === 'company-2' ? 'Abuja Health Network' : 'Lagos Fintech Group' } }; demoCrmTasks.unshift(task); return demoResponse(config, task, 201); }
  if (method === 'get' && url.startsWith('/admin/crm/lifecycle')) return demoResponse(config, demoCrmLifecycle);
  if (method === 'post' && url === '/admin/crm/lifecycle') { const event = { id: crypto.randomUUID(), ...body, occurredAt: body.occurredAt || now, clientCompany: { id: body.clientCompanyId, name: body.clientCompanyId === 'company-2' ? 'Abuja Health Network' : 'Lagos Fintech Group' } }; demoCrmLifecycle.unshift(event); return demoResponse(config, event, 201); }
  if (method === 'post' && url === '/admin/crm/renewals') { const renewal = { id: crypto.randomUUID(), stage: 'DISCOVERY', probability: 50, contractValue: 0, ...body, clientCompany: { id: body.clientCompanyId, name: body.clientCompanyId === 'company-2' ? 'Abuja Health Network' : 'Lagos Fintech Group', securityPosture: { score: 75 }, subscription: { status: 'ACTIVE' } } }; demoRenewalOpportunities.unshift(renewal); return demoResponse(config, renewal, 201); }
  if (method === 'get' && url.startsWith('/admin/crm/timeline')) return demoResponse(config, [...demoMessages.map(item => ({ id: item.id, type: 'Communication', client: item.clientCompany.name, title: item.subject, detail: item.channel, at: item.createdAt })), ...demoMeetings.map(item => ({ id: item.id, type: 'Meeting', client: item.clientCompany.name, title: item.title, detail: item.type, at: item.startsAt })), ...demoCrmTasks.map(item => ({ id: item.id, type: 'CRM Task', client: item.clientCompany.name, title: item.title, detail: `${item.priority} / ${item.status}`, at: item.createdAt })), ...demoCrmLifecycle.map(item => ({ id: item.id, type: 'Lifecycle', client: item.clientCompany.name, title: item.summary, detail: item.stage, at: item.occurredAt }))]);
  if (method === 'post' && url === '/admin/csr') {
    const existing = demoCsr.find(item => item.clientCompanyId === body.clientCompanyId);
    const record = { ...(existing || { id: crypto.randomUUID(), clientCompany: { id: body.clientCompanyId, name: 'Managed Client', subscription: { plan: 'pro' }, securityPosture: { score: 75 } } }), ...body };
    if (existing) Object.assign(existing, record); else demoCsr.unshift(record);
    return demoResponse(config, record, 201);
  }
    ...
    return demoResponse(config, {
      ...
      attackLab: {
        ...
        mostTriggeredScenarios: [
          { id: 'scenario-phishing-campaign', title: 'Phishing Campaign Exercise', _count: { runs: 2 } },
          { id: 'scenario-credential-stuffing', title: 'Credential Stuffing Attempt', _count: { runs: 2 } },
          { id: 'scenario-web-app-probing', title: 'Web App Probing', _count: { runs: 2 } }
        ],
        readinessScores: [{ client: 'Lagos Fintech Group', score: 84 }]
      },
