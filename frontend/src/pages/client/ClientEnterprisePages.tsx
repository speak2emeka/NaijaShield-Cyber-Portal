...
export function ClientBilling() {
  ...
  useEffect(() => {
    ...
    api.get('/billing/client/subscription').then(({ data }) => setSubscription(data));
  }, []);

  async function checkout(planId: string) {
    const { data } = await api.post('/billing/checkout', { planId, provider: 'STRIPE' });
  ...
  return (
    <div className="space-y-6">
      ...
      {subscription && <section className="glass-card p-5"><h2 className="text-xl font-bold">Current Plan</h2><div className="mt-3 grid gap-3 md:grid-cols-4"><MetricCard label="Plan" value={subscription.plan} /><MetricCard label="Status" value={subscription.status} /><MetricCard label="Interval" value={subscription.billingInterval || 'monthly'} /><MetricCard label="Invoices" value={subscription.invoices?.length || 0} /></div></section>}
