...
import { Bot, ClipboardList, FileText, GitBranch, Radar, SearchCode, ShieldAlert, UploadCloud } from 'lucide-react';
...
export function AiSecurityAssistant() {
  ...
  async function run(endpoint: string) {
    const payload = { architecture: { name: 'NaijaShield client portal', stack: ['React', 'Express', 'PostgreSQL'] }, assets: [{ identifier: 'client-portal', type: 'APP', riskLevel: 'MEDIUM' }] };
    const { data } = await api.post(endpoint, payload);
    setOutput(data);
  }
  return <div className="space-y-6"><h1 className="flex items-center gap-2 text-2xl font-black"><Bot className="text-shield-glow" />AI Security Assistant</h1><div className="grid gap-3 md:grid-cols-4"><button className="btn-primary" onClick={() => run('/admin/ai/threat-model')}>Threat Model</button><button className="btn-secondary" onClick={() => run('/admin/ai/attack-surface/analyze')}>Attack Surface</button><button className="btn-secondary" onClick={() => run('/admin/ai/test-cases/generate')}>Test Cases</button><button className="btn-secondary" onClick={() => run('/admin/ai/vuln/analyze')}>Vuln Analysis</button></div>{output && <JsonPanel data={output} />}</div>;
}
...
export function ScanResultsDashboard() {
  ...
  async function runScan(event: FormEvent<HTMLFormElement>) {
    ...
    toast.success('Safe scan orchestration dry-run completed');
    refresh();
  }
  return <div className="space-y-6"><h1 className="flex items-center gap-2 text-2xl font-black"><SearchCode className="text-shield-glow" />Scan Results</h1><form onSubmit={runScan} className="glass-card grid gap-3 p-4 md:grid-cols-4"><select className="input" name="tool"><option>ZAP</option><option>NMAP</option><option>SEMGREP</option><option>DEPENDENCY</option></select><input className="input" name="target" placeholder="Approved target" required /><input className="input" name="clientCompanyId" placeholder="Client ID optional" /><button className="btn-primary" type="submit">Run Dry Scan</button></form><DataTable headers={['Tool', 'Target', 'Status', 'Client', 'Summary']}>{data.map((run: any) => <tr key={run.id}><td className="px-5 py-4 font-bold">{run.tool}</td><td className="px-5 py-4">{run.target}</td><td className="px-5 py-4">{run.status}</td><td className="px-5 py-4">{run.clientCompany?.name || '-'}</td><td className="px-5 py-4">{JSON.stringify(run.summary?.counts || {})}</td></tr>)}</DataTable></div>;
}
